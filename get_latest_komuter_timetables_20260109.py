...
def get_ktmb_komuter_timetables():
    ...
    try:
        ...
        for link in soup.find_all('a', attrs={'data-target': '#reusemodal'}):
            ...
            title = link.find('b').get_text(strip=True) if link.find('b') else pd.NA
            effective_date = title.split('Effective ')[1] if 'Effective' in title else pd.NA
...
# Step 1: Replace 'NA' string with real NaN
# Apply the function to create a new 'Effective_Date' column

timetables_df['Effective'] = timetables_df['Effective'].replace({'NA', 'N/A', 'null', ''}, pd.NA)
timetables_df['Effective'] = pd.to_datetime(timetables_df['Effective'], errors='coerce')

# Step 4: Fill missing Effective dates using PDF link
print(f"Filling missing Effective dates using PDF links...")
timetables_df['Effective'] = timetables_df.apply(
    lambda row: extract_date_from_link(row['PDF Links']) 
    if pd.isna(row['Effective']) else row['Effective'],
    axis=1
)
...
latest_utara = north_filtered_timetables.nlargest(1, 'Effective')
